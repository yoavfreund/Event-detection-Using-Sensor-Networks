...
video cameras with partially overlapping fields of view one can reconstruct temporal sequences in three dimensions and from multiple view-points. However, connecting sensors to each other and to electricity with wires is often impractical, which severely limits sensor networks in terms of energy and communication bandwidth. In particular, it is impractical for each sensor to send all of it's raw data to a central {\em fusing} computer.  

A leading approach to alleviate this problem is to {\em push the computation to the edge of the network}. In other words
perform computation at the sensor to decrease the amount of data that the sensor sends out. 
This proposal focuses on pushing data science to the sensors themselves.  We propose 

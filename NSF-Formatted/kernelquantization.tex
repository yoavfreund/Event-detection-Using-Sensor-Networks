...
in~\ref{item:monitoring}.  Here a network of sensors is used to
...
\cite{gretton2012kernel,gribonval2017compressive}
...
The corresponding signal at sensor $i$, $Y_i = \transfer_i \mathbf{X}$ can then be quantized via ${Q}(\transfer_i \mathbf{X})$ with a quantization map $Q: \mathbb{R}^{m_i} \to \{\pm 1\}^{q_i} $ that maps the measurements to bits.  The full quantization map of all sensors can be represented as

...
Henceforth, the map at sensor $i$, $\transfer_i$, will be a slight modification of the transfer function $\Phi_i$ presented in \eqref{eq:transfer}.  Here, we include a randomized embedding of the digitized signal at each sensor.  To overload notation, $\Phi_i \leftarrow \mathcal{A}\Phi_i$, where $\mathcal{A}$ is a randomized linear (or nonlinear) Johnson-Lindenstrauss map.
%
%
...
The corresponding signal at sensor $i$, $\mathbf{Y}_i = \transfer_i (\mathbf{X})$ can then be quantized via ${Q}(\transfer_i (\mathbf{X}))$ with a quantization map $Q: \mathbb{R}^{m_i} \to \{\pm 1\}^{m_i} $ that maps the measurements to bits.  The full quantization map of all sensors can be represented as
...
approximates an arbitrary kernel, for example the Gaussian kernel $\langle g(x), g(y) \rangle =
